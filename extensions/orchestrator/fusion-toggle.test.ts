...
describe("global fusion toggle", () => {
	...
	it("omits fusion from active tools and prompt section when disabled", async () => {
		...
		expect(pi.getActiveToolsHistory()[0]).toEqual(["plan", "plan_add_steps", "insert_step", "delegate", "read_skill", "list_skills", "list_tools", "vision_query"]);
	...
	it("includes fusion in active tools and prompt section when enabled", async () => {
		...
		expect(pi.getAllTools().some((t: any) => t.name === "fusion")).toBe(true);
		expect(pi.getActiveToolsHistory()[0]).toEqual(["plan", "plan_add_steps", "insert_step", "delegate", "fusion", "read_skill", "list_skills", "list_tools", "vision_query"]);
