...
describe("createFlightRecorderDump", () => {
	...
	it("includes blocked calls and token summary", () => {
		const params = {
			...
			tokenSummary: { totalInput: 500, totalOutput: 200, totalCached: 100, cacheWrite: 50, ctxTokensFinal: 800 },
		};
...
describe("C1: live token accumulator", () => {
	...
	it("accumulates token usage from 3 assistant message_end events", { timeout: 10_000 }, async () => {
		...
		expect(d.tokenCached).toBe(175);    // cumulative: 50+100+25
	});
	...
	it("captures ctxTokens from totalTokens", { timeout: 10_000 }, async () => {
		...
		expect(d.ctxTokens).toBe(15000); // uses totalTokens directly
	});
