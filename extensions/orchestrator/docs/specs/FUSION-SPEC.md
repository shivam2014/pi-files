...
When `enabled: false`:
- The fusion tool is always registered at init (idempotent), but excluded from active tools via `setActiveTools(["plan", "delegate"])`
- No system prompt instructions about fusion are injected
- The orchestrator workflow is identical to before Fusion existed

When `enabled: true`:
- Fusion tool is registered at init and included in `setActiveTools(["plan", "delegate", "fusion"])`
...
```typescript
// registration-hub.ts — all tools registered at init
registerFusionTool(pi, cwd);  // always registered; visibility controlled below

// index.ts — before_agent_start controls visibility via setActiveTools
const activeTools = ["plan", "delegate"];
if (fusionEnabled) activeTools.push("fusion");
pi.setActiveTools(activeTools);
```

The `registerFusionTool()` function always registers the fusion tool at init (idempotent). The `/fusion` command stores session state in a module-level variable. `setActiveTools` in `before_agent_start` controls whether the tool is visible to the orchestrator — when disabled, the tool exists but is not in the active tool list.
...
|-------|------|---------|-------------|
| `enabled` | boolean | `true` | Master switch for the Fusion tool. When false, tool is registered but excluded from active tools via setActiveTools. |
