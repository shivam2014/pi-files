...
describe("registerPlanTool", () => {
	...
	it("derives goal from steps when goal is empty", async () => {
		...
		expect(result.details.goal).toBe("Untitled plan");
		expect(setupPlanPanel).toHaveBeenCalledWith("Untitled plan", steps, ctx);
	});
	...
	it("renderCall derives goal from steps when goal is empty", () => {
		...
		expect((text as any).text).toContain("Untitled plan");
	});
});
